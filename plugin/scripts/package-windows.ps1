...
$candidatePluginPaths = if ($BuildDirectory) {
    ...
    @(
        (Join-Path $resolvedBuildDirectory "plugin\LoopSurgeon_artefacts\Release\VST3\loopy.vst3"),
        (Join-Path $resolvedBuildDirectory "LoopSurgeon_artefacts\Release\VST3\loopy.vst3")
    )
} else {
    @(
        (Join-Path $buildRoot "vs2022\plugin\LoopSurgeon_artefacts\Release\VST3\loopy.vst3"),
        (Join-Path $buildRoot "loopy\plugin\LoopSurgeon_artefacts\Release\VST3\loopy.vst3")
    )
}
$pluginPath = $candidatePluginPaths | Where-Object {
    Test-Path -LiteralPath $_ -PathType Container
} | Select-Object -First 1
$guidePath = Join-Path $repositoryRoot "docs\WINDOWS_REAPER_TEST.md"
$packageRoot = Join-Path $buildRoot "packages"
$packageName = "loopy-$Version-Windows-x64-VST3"
...
$pluginBinary = Join-Path $stagePath "loopy.vst3\Contents\x86_64-win\loopy.vst3"
if (-not (Test-Path -LiteralPath $pluginBinary -PathType Leaf)) {
    throw "Packaged VST3 binary is missing: $pluginBinary"
}

$moduleInfoPath = Join-Path $stagePath "loopy.vst3\Contents\Resources\moduleinfo.json"
