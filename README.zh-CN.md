...
| 更适合 | 需要保留原始时间线和运动过程的声音 | 需要稳定能量、重塑时间流动，同时保留素材身份的连续声音 |

`LOOP` 自动完成声音设计师制作 loop 时通常要进行的操作：选择可用素材、确定循环
边界、建立连续接缝，并让结果适配所需的最终时长。目标较短时，它会在选区中寻找
边界条件良好的精确长度周期；目标较长时，它会建立一个可循环的基础周期，再重复完整
周期，使最终边界回到相同相位。

`TEXTURE` 直接使用选定录音构建连续纹理。它可以弱化宏观包络和通过感、控制较短
尺度上反复出现的动态，并在环形时间线上衔接相位相容的区域来重塑时间流动。结果保留
源素材的声音特征，同时获得更稳定、较少受原始事件顺序限制的纹理形态。
