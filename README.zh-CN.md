...
loopy 先按目标时长搜索可用区间，并比较波形、音量、相位和左右声道在接缝处是否连续。
选出内部切点后，它把原始首尾接缝移到中间做交叉淡化，再将完成的基础 loop 延伸到
精确目标时长。
...
loopy 先减弱选区中缓慢的大起大落，再把素材划分为互相重叠的区域。每次优先选择接续
平滑、音量稳定且近期没有使用过的区域，并让末端重叠接回开头；之后再应用可选的
Crush 和 Character 处理。
