# Loop Surgeon macOS / REAPER test

An Apple Silicon build has not been produced on this Windows machine. After a local arm64 macOS
build, copy `Loop Surgeon.vst3` to `~/Library/Audio/Plug-Ins/VST3/`, rescan in REAPER and test both
Rotate & Repair and Texture Loop using the same workflow as the Windows guide.
